package com.jamify_engine.engine.utils;


public class Constants {

    public static final String ISSUER_URI = "https://test-issuer.com";
    public static final String KEY_ID = "test-key-id";
    public static final String TEST_USER_EMAIL = "test-user@example.com";
    public static final String TEST_USER_EMAIL_2 = "test-user-2@example.com";
    public static final String TEST_EXPIRED_USER_EMAIL = "test-expired-user@example.com";
    public static final String TEST_PROVIDER = "spotify";
    public static final String TEST_USER_NAME = "John Doe";
    public static final String FRANCE_ALPHA_CODE2 = "FR";
    public static final String JAM_NAME_TEST = "Jam name test";
    public static final String ACCESS_TOKEN_TEST = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..RLN_GL4S8Me7yEFLdGr6wTf-AweABF34gHL0fHjnaoxyaqitcvjjdJLeVl-K6Y5fVxZjX1RMlfY5i-hfbHTAn3g_Wy6zD8v05vVfdwgqQzT5ZrzQlDFg73ogHhRYRMfep0xFTYIOe1uhvMu2CWT2KHdc20VE32sapwzoVdkUXU8EKES3scYOkf1hA_rzQDi64afRjoN-1UEpOJNOiwJ0wk51GXvI7tLXkBeKBwz2DBXgTHDrblj5uyBx8lICxGzuKy8-5k9Mjk4R5EtpDHIH9ILhdTIb5O3u8iJnWrr3Lykxj4u748VA7I6ZsraUNXf2fNqlkrx0dppn05dqz-uobA";
}
